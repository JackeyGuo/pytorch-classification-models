...
class CheckpointSaver:
    ...
    def save_checkpoint_raw(self, epoch, metric=None):
        assert epoch >= 0
    ...
    def save_checkpoint(self, epoch, metric=None):
        ...
        if self.cmp(metric, self.best_metric):
            best_save_path = os.path.join(self.checkpoint_dir, 'best' + self.extension)

            filename = '-'.join([self.save_prefix, str(epoch)]) + self.extension
            save_path = os.path.join(self.checkpoint_dir, filename)
            if len(self.checkpoint_files) > 0:
                os.remove(self.checkpoint_files[0])
                self.checkpoint_files = []
            self.checkpoint_files.extend([save_path, metric])

            self.best_epoch = epoch
            self.best_metric = metric
            # self._save(save_path, epoch, metric)
            torch.save(get_state_dict(self.model, self.unwrap_fn), save_path)

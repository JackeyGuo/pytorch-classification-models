...
class LetterBoxResize:
    ...
    def __call__(self, img):
        ...
        img = F.resize(img, (resize_h, resize_w), interpolation=self.interpolation)
...
class imgPadding:
    ...
    def __call__(self, img):
        ...
        max_size = max(h, w)
        new_img = np.zeros((max_size, max_size, 3), np.uint8)

        if h > w:
            new_h = max_size
            new_w = int(w * max_size * 1.0 / h)
            img = F.resize(img, (new_h, new_w), self.interpolation)
            start_x = (max_size - new_w) // 2
            start_y = 0
        else:
            new_w = max_size
            new_h = int(h * max_size * 1.0 / w)
            img = F.resize(img, (new_h, new_w), self.interpolation)
            start_x = 0
            start_y = (max_size - new_h) // 2

        new_img[start_y:start_y + new_h, start_x:start_x + new_w, :] = img
        new_img = Image.fromarray(new_img)
        img = F.resize(new_img, (new_height, new_width), self.interpolation)
        return img
...
class RandomRotate(object):
    ...
    def __call__(self, img):
        if random.random() < self.p:
            ...
            img = img.rotate(rotate_degree, Image.BICUBIC)
        return img
...
class RandomChoiceRotate:
    ...
    def __init__(self, p=0.5):
        self.rotate_angle = [90, -90]
        self.p = p

    def __call__(self, img):
        if random.random() < self.p:
            angle = random.choice(self.rotate_angle)
            img = F.rotate(img, angle, Image.BICUBIC)
        return img
